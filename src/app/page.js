...
const ProductCard = ({ p, badge }) => (
  <div
    ...
    className="relative bg-white shadow-md rounded-xl overflow-hidden p-2 flex flex-col border border-gray-100 transition-all duration-200 hover:shadow-xl hover:-translate-y-1 group min-h-[240px] cursor-pointer"
  >
    <span className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-[10px] font-semibold shadow
    ...
    <h2 className="font-semibold text-[11px] text-gray-800 mb-1 line-clamp-2">{p.title}</h2>
    <div className="flex items-center gap-1 mb-1">
      <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">{p.category}</span>
    ...
    <p className="text-gray-500 text-[10px] mb-1 line-clamp-2">{p.description}</p>
    <div className="flex items-center justify-between mt-auto gap-1">
      ...
      <button
        className="bg-blue-500 hover:bg-blue-600 text-white px-2 py-0.5 rounded text-[10px] flex items-center gap-1"
...
<div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {getMultipleOfFour(filteredProducts).map((p) => (
                    <ProductCard key={p.id} p={p} badge={selectedCategory ? selectedCategory : "Hot"} />
                  ))}
                </div>
              </section>
            </main>
